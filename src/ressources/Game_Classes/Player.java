...
public class Player {
    ...
    public void placeCard(Slot placemenSlot, int index){
        if (placemenSlot == null){
            this.playerHand.placeCard(placemenSlot, index, this.game, this);
        }
    }
    ...
    // public Card getCardWithEffect(Card_Controller.Effects effect){
    //     for(Slot slot: this.Persons_Field){
    ...
    // public boolean isEffectInPersons(Card_Controller.Effects effect){
    //     for(Slot slot: this.Persons_Field){
    //         Card card = slot.getCard();
    //         if(card.status.contains(effect)){
    //             return true;
    ...
    // public boolean isEffectInEvent(Card_Controller.Effects effect){
    //     for(Slot slot: this.Event_Slots){
    //         Card card = slot.getCard();
    //         if(card.status.contains(effect)){
    //             return true;
    //         }
    ...
    //Set the You Card 
    public void setYouCard(MajCard You_Card){
        You_Card.setHealth(50);
        You_Card.setCost(0);
    ...
    public boolean isCardInEvent(Card card){
        for(Slot slot : this.Event_Slots){
            if (Objects.equals(slot.getCard(), card)){
                return true;
            }
